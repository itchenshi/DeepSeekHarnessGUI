...
**发布说明：** 自 v0.2.0 以来的功能版本。打包不再每轮重新下载 Node / Electron；第三方插件的「安装」与「启用」拆成两个独立状态并全部与插件市场双向同步；用量插件加入 DeepSeek 账户余额，更名为「模型用量与余量」；应用图标底色改为白色。
...
- `build/icon.ico` 改由 `scripts/make-icons.mjs` 直接生成，不再依赖打包工具从 PNG 转换。
- 附 `scripts/ico-info.cjs` / `scripts/exe-icon-info.cjs`，可随时自查 `.ico` 与 exe 内嵌图标的构成。
...
> `DSH.GUI.Setup.0.3.0.exe`、`DSH.GUI-0.3.0-win.zip` 这种形式（本地构建产物为空格版）。
> 内容完全一致，按页面上的名字下载即可。
