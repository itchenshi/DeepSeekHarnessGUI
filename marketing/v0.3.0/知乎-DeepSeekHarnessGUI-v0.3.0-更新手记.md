# DSH GUI v0.3.0 更新手记：图标重画了、打包快了，插件也终于分清了「装着」和「开着」
...
3. **Windows 图标不够显眼。** 摆在桌面和快捷方式里缺少辨识度，别人第一眼看不出这是谁。这一条最伤——它是别人认识这个项目的第一眼。
...
## 四、第三件事：Windows 图标，换成白底圆角 + 品牌蓝

这一条技术上最琐碎，收益上最直接——因为它决定别人点开你的项目之前看到什么。

- **主图标重画为「白色圆角方块 + 品牌蓝字形」**（官方 `#4D6BFE`，带极浅渐变与细描边）。摆在桌面和快捷方式上都更醒目，深色浅色壁纸下都清楚。
- **图标改为直接生成**：不再让打包工具从 PNG 转换，`scripts/make-icons.mjs` 直接把官网 favicon 渲染成 `build/icon.ico`。
- 另附两个可以随时自查的小工具：`scripts/ico-info.cjs`（看任意 .ico 的帧与长度）和 `scripts/exe-icon-info.cjs`（看 exe 内嵌的 `RT_ICON` / `RT_GROUP_ICON`）。

一个使用提醒：改完图标要**重新安装或重建快捷方式**。如果之后还显示旧的图标，重启一次资源管理器（或删掉 `%LocalAppData%\IconCache.db`）刷新一下图标缓存就好。
...
从 0.1.0 到 0.3.0，我的取舍没变过一条：**壳只做官方引擎没空做的「体验活」，内核永远 100% 官方**。所以这个版本里最花时间的其实不是新功能，而是那些「看起来不算功能」的地方——一个更醒目的图标、一次不用等下载的打包、一个不会说谎的开关状态。这些东西没法写进功能列表里炫耀，但它们是每天都在用的部分。
