...
> 截图建议（均为深色主题，文件就在本目录）：
> - `主窗口.png` — 放在开头的 hero 图和「痛点故事 / 它解决了什么」之后，展示主窗口与左侧的工作区、会话列表；
> - `设置.png` — 放在「它解决了什么」清单之后，展示左列（关闭窗口 / 数据目录 / 引擎更新）与右列第三方插件的「安装勾选框 + 启用开关」；
> - `DSH设置.png` — 放在「v0.3.0 值得单独说的三件事」之后，展示 Harness 页面内的设置与插件市场；
> - `模型OpenCodeGo余量.png` — 放在「30 秒上手」之后，展示会话标题右侧的模型用量面板（OpenCode Go 滚动 / 周 / 月）；
> - `模型DeepSeek余量.png` — 需要强调 DeepSeek 账户余额时替换上一张（两张模型用量截图均为中文界面）；
> - `设置-english.png` — 放在「适合谁 / 不适合谁」之后，说明界面为跟随系统 / 中文 / English；
> - 英文界面的 `主窗口-english.png`、`设置-english.png`、`DSH设置-english.png` 可按需替换上面几张（两张模型用量截图只有中文界面）。
